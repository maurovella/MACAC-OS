...
int piped_process_handle(char ** params, unsigned int params_size) {
	...
	char * params_1[2] = {params[1], NULL};
	char * params_2[2] = {params[2], NULL};
	sys_create_child_process(params_1, programs[program_1].base_priority, STDIN, pipe_id, programs[program_1].function_ptr);
	do_print_color("First process created\n", GREEN);
	sys_create_child_process(params_2, programs[program_2].base_priority, pipe_id, STDOUT, programs[program_2].function_ptr);
